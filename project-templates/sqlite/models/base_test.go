...
const dbpath = "$GO_BOOTSTRAP_GOPATH/$GO_BOOTSTRAP_REPO_NAME/$GO_BOOTSTRAP_REPO_USER/$GO_BOOTSTRAP_PROJECT_NAME/"
...
func newDbForTest(t *testing.T) *sqlx.DB {
	defaultDSN := strings.Replace(dbpath + "$GO_BOOTSTRAP_PROJECT_NAME_test.db", "-", "_", -1)
	db, err := sqlx.Connect("sqlite3", defaultDSN)
	if err != nil {
		t.Logf(defaultDSN)
		t.Fatalf("Connecting to SQLite3 DB should never fail. Error: %v", err)
	}
	return db
}
